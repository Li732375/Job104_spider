...
2. **資料輸出至 CSV**
   爬取完所有職缺詳細資料後，將資料存至 `E04jobs_日期.csv`。
...
參見 [wiki](https://github.com/Li732375/JobE04_spider/wiki) 列表，提供各項參數自行調整
...
4. `E04jobs_日期.csv` 檔名只到日，當天多次執行會覆蓋舊檔。
5. 若表格文字自動換行，可手動取消「自動換行」避免單元格過高。

---

> 覺得 `E04jobs_?.csv` 資料格子太高？ 全選表格 > 儲存格格式 > 對齊方式 > 文字控制 > 取消勾選 "自動換行"。

4.  `E04jobs_?.csv` 因為檔名時間僅寫到 **日**，同一天的會被覆蓋掉喔！
